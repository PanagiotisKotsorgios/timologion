...
const RULES: Rule[] = [
  ...
  // ── Classification mismatches ─────────────────────────────────────
  {
    ...
    el: "Ο συνδυασμός κατηγοριοποίησης εσόδου δεν είναι έγκυρος για αυτόν τον τύπο παραστατικού (myDATA απορρίπτει τον συνδυασμό). Στείλε στην υποστήριξη το ID του παραστατικού + το πλήρες αγγλικό μήνυμα (Wrapp code + category + type) για να διορθώσουμε τη χαρτογράφηση.",
  },
  {
    match:
      /could not load.*valid validation doc for classification with category (\S+) and type (\S+)/i,
    el: "Ο τύπος παραστατικού δεν επιτρέπει τους κωδικούς κατηγορίας/τύπου που στέλνουμε στο myDATA. Στείλε στην υποστήριξη το ID του παραστατικού και το πλήρες αγγλικό μήνυμα ώστε να διορθώσουμε το mapping.",
  },
  {
    match:
